...
// Manual "Close" buttons on open paper/futures positions - close at the live price.
function bindCloseButtons(containerId) {
  const c = $(containerId); if (!c) return;
  c.querySelectorAll("[data-close]").forEach((b) => (b.onclick = async () => {
    const path = b.dataset.close, id = b.dataset.id;   // path = "paper" | "futures"
    if (!confirm("Close this trade now at the live market price?")) return;
    b.disabled = true; b.textContent = "…";
    try {
      const r = await api2(`/api/${path}/close`, { id: Number(id) });
...
async function loadSettings() {
  try {
    ...
    if ($("set-paperapproval")) $("set-paperapproval").checked = !!s.paperApproval;
    ...
    if ($("gd-bt")) $("gd-bt").checked = !!s.backtestGate;
    if ($("gd-btwin") && s.backtestMinWin != null) $("gd-btwin").value = s.backtestMinWin;
    if ($("gd-bttr") && s.backtestMinTrades != null) $("gd-bttr").value = s.backtestMinTrades;
    if ($("gd-univ") && s.universeSize != null) $("gd-univ").value = s.universeSize;
...
async function loadPaper() {
  ...
  $("paper-cards").innerHTML = [
    ...
    card("Free cash", "$" + p.cashUsd),
    card("Invested", "$" + p.investedUsd),
    card("If all TP1 hit", "+$" + tp1Target, "text-emerald-400"),
  ...
  $("paper-open").innerHTML = p.open.length
    ? `<thead><tr class="text-left text-xs uppercase text-slate-500"><th>Coin</th><th>Quality</th><th class="text-right">Buy</th><th class="text-right">Live</th><th class="text-right">TP1</th><th class="text-right">Stop</th><th class="text-right">Size $</th><th class="text-right">Qty</th><th class="text-right">Value $</th><th class="text-right">P/L now</th><th class="text-right">TP1 → profit</th><th class="text-right">Est. TP1 (SL)</th><th>Bought (SL)</th><th></th></tr></thead><tbody>${p.open.map((x) => `<tr class="border-b border-edge/60"><td class="py-1.5 font-semibold">${x.symbol}</td><td class="py-1.5 text-xs text-slate-400">${x.quality || "-"}</td><td class="py-1.5 text-right tabular-nums">${usd(x.entry_price)}</td><td class="py-1.5 text-right tabular-nums">${x.livePrice == null ? "-" : usd(x.livePrice)}</td><td class="py-1.5 text-right tabular-nums text-emerald-300">${usd(x.tp1)}</td><td class="py-1.5 text-right tabular-nums text-rose-300">${usd(x.stop)}</td><td class="py-1.5 text-right tabular-nums">${x.cost_usd}</td><td class="py-1.5 text-right tabular-nums text-slate-400">${x.qty}</td><td class="py-1.5 text-right tabular-nums">${x.marketValueUsd == null ? "-" : x.marketValueUsd}</td><td class="py-1.5 text-right tabular-nums ${g(x.unrealizedUsd)}">${x.unrealizedUsd == null ? "-" : (x.unrealizedUsd >= 0 ? "+" : "") + x.unrealizedUsd + " (" + (x.unrealizedPct >= 0 ? "+" : "") + x.unrealizedPct + "%)"}</td><td class="py-1.5 text-right tabular-nums text-emerald-400">+$${x.tp1ProfitUsd} <span class="text-[11px] text-slate-500">(+${x.tp1Pct}%)</span></td><td class="py-1.5 text-right text-xs ${x.etaLeftMin != null && x.etaLeftMin < 0 ? "text-amber-400" : "text-slate-300"}">${x.etaTp1SL || "-"}${x.etaLeftMin != null ? `<br><span class="text-[11px] text-slate-500">${x.etaLeftMin > 0 ? "in ~" + x.etaLeftMin + "m" : "overdue"}</span>` : ""}</td><td class="py-1.5 text-xs text-slate-400">${slTime(x.opened_at)}</td><td class="py-1.5"><button data-close="paper" data-id="${x.id}" class="rounded border border-rose-700/60 px-2 py-0.5 text-xs text-rose-300 hover:bg-rose-900/30">Close</button></td></tr>`).join("")}</tbody>`
    : `<tbody><tr><td class="py-3 text-slate-500">No open positions. A good ≥95% coin is bought automatically${p.enabled ? "" : " (turn on Auto-trade above)"}.</td></tr></tbody>`;
  bindCloseButtons("paper-open");
...
async function loadFutures() {
  ...
  $("fut-open").innerHTML = p.open.length
    ? `<thead><tr class="text-left text-xs uppercase text-slate-500"><th>Coin</th><th>Side</th><th class="text-right">Entry</th><th class="text-right">Live</th><th class="text-right">TP</th><th class="text-right">Stop</th><th class="text-right">Liq</th><th class="text-right">Margin</th><th class="text-right">Notional</th><th class="text-right">P/L now</th><th class="text-right">TP → profit</th><th class="text-right">Est. TP (SL)</th><th>Opened (SL)</th><th></th></tr></thead><tbody>${p.open.map((x) => `<tr class="border-b border-edge/60"><td class="py-1.5 font-semibold">${x.symbol}</td><td class="py-1.5 ${x.direction === "LONG" ? "text-emerald-400" : "text-rose-400"}">${x.direction} ${x.leverage}x</td><td class="py-1.5 text-right tabular-nums">${usd(x.entry_price)}</td><td class="py-1.5 text-right tabular-nums">${x.livePrice == null ? "-" : usd(x.livePrice)}</td><td class="py-1.5 text-right tabular-nums text-emerald-300">${usd(x.tp1)}</td><td class="py-1.5 text-right tabular-nums text-rose-300">${usd(x.stop)}</td><td class="py-1.5 text-right tabular-nums text-amber-400">${usd(x.liqPrice)}</td><td class="py-1.5 text-right tabular-nums">${x.cost_usd}</td><td class="py-1.5 text-right tabular-nums text-slate-400">${x.notional_usd}</td><td class="py-1.5 text-right tabular-nums ${g(x.unrealizedUsd)}">${x.unrealizedUsd == null ? "-" : (x.unrealizedUsd >= 0 ? "+" : "") + x.unrealizedUsd + " (" + (x.unrealizedPct >= 0 ? "+" : "") + x.unrealizedPct + "%)"}</td><td class="py-1.5 text-right tabular-nums text-emerald-400">+$${x.tp1ProfitUsd}</td><td class="py-1.5 text-right text-xs ${x.etaLeftMin != null && x.etaLeftMin < 0 ? "text-amber-400" : "text-slate-300"}">${x.etaTp1SL || "-"}${x.etaLeftMin != null ? `<br><span class="text-[11px] text-slate-500">${x.etaLeftMin > 0 ? "in ~" + x.etaLeftMin + "m" : "overdue"}</span>` : ""}</td><td class="py-1.5 text-xs text-slate-400">${slTime(x.opened_at)}</td><td class="py-1.5"><button data-close="futures" data-id="${x.id}" class="rounded border border-rose-700/60 px-2 py-0.5 text-xs text-rose-300 hover:bg-rose-900/30">Close</button></td></tr>`).join("")}</tbody>`
